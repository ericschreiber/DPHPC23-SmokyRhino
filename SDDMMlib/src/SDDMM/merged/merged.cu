    ...
    const int num_tiles)
{
    ...
    // const int bdim = blockDim.x; // Trying to reduce the num registers used.
    ...
    {
        const int col_offset = (matrixC_GPU_col_indices[elem_index] * k) +  // the thing in parens is ptr to start of col of B that we are computing the dot prod with
