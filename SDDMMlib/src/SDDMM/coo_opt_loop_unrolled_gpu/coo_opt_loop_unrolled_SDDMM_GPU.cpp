// coo_opt_loop_unrolled_SDDMM_GPU.cpp
...
void coo_opt_loop_unrolled_SDDMM_GPU<float>::SDDMM_COO(
...
void coo_opt_loop_unrolled_SDDMM_GPU<float>::SDDMM(
    const DenseMatrix<float>& matrixA_HOST,
    const DenseMatrix<float>& matrixB_HOST,
...
void coo_opt_loop_unrolled_SDDMM_GPU<float>::start_run() const
{
    assert(this->_timer != nullptr && "Error: coo_opt_loop_unrolled_SDDMM_GPU::start_run() timer is nullptr. Check that you have set the timer with <SDDMM>.set_timer()");
    this->_timer->start_gpu_run();
}

void coo_opt_loop_unrolled_SDDMM_GPU<float>::stop_run() const
{
    this->_timer->stop_gpu_run();
}

// Explicit template instantiation
template class coo_opt_loop_unrolled_SDDMM_GPU<double>;
template class coo_opt_loop_unrolled_SDDMM_GPU<int>;