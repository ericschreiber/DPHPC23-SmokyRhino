...
void tiled_tiles<float>::SDDMM_COO(
...
void tiled_tiles<float>::start_run() const
{
    assert(this->_timer != nullptr && "Error: tiled_tiles::start_run() timer is nullptr. Check that you have set the timer with <SDDMM>.set_timer()");
    this->_timer->start_cpu_run();
}

void tiled_tiles<float>::stop_run() const
{
    this->_timer->stop_cpu_run();
}

// Explicit template instantiation
template class tiled_tiles<double>;
template class tiled_tiles<int>;