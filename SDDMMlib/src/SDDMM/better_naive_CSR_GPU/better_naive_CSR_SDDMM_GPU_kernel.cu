    ...
    float* d_result)
{
    ...
    for (int i = blockIdx.x * rows_per_block + warp_idx; i < m; i += grid_size)
    {
        // if (i == 2)
        // {
        //     printf("Hello");
    ...
    float* d_result)
{
    ...
    for (int i = blockIdx.x * rows_per_block + (warp_idx / warps_per_line); i < m; i += grid_size)
    {
        // iterate over all elements that need to be computed in that row
        for (int j = d_rowPtr[i]; j < d_rowPtr[i + 1]; j++)
        {
            ...
            {
                block_sum = two_warp_per_line_reduction(warpSums_buffer[thread_id]);
            }
    ...
    float* d_result)
{
    ...
    for (int i = blockIdx.x * rows_per_block + (warp_idx / warps_per_line); i < m; i += grid_size)
    {
        ...
        for (int j = d_rowPtr[i]; j < d_rowPtr[i + 1]; j++)
        {
            ...
            if ((warp_idx % warps_per_line) == 0)
            {
                block_sum = four_warp_per_line_reduction(warpSums_buffer[thread_id]);
            }
    ...
    float* d_result)
{
    ...
    for (int i = blockIdx.x * rows_per_block + (warp_idx / warps_per_line); i < m; i += grid_size)
    {
        ...
        for (int j = d_rowPtr[i]; j < d_rowPtr[i + 1]; j++)
        {
            ...
            if ((warp_idx % warps_per_line) == 0)
            {
                block_sum = eight_warp_per_line_reduction(warpSums_buffer[thread_id]);
            }
    ...
    float* d_result)
{
    ...
    for (int i = blockIdx.x * rows_per_block + (warp_idx / warps_per_line); i < m; i += grid_size)
    {
        ...
        for (int j = d_rowPtr[i]; j < d_rowPtr[i + 1]; j++)
        {
            ...
            if ((warp_idx % warps_per_line) == 0)
            {
                block_sum = sixteen_warp_per_line_reduction(warpSums_buffer[thread_id]);
            }
...
void compute_blockwise(
    ...
    float* d_result)
{
    std::cout << lines_per_block << ", " << warps_per_line << std::endl;
    ...
    else
    {
        if (warps_per_line == 2)
        {
            ...
            two_warps_per_line_SDDMM_kernel<<<160, 1024>>>(
                2,
                m,
                n,
                k,
        ...
        else if (warps_per_line > 2 && warps_per_line < 5)
        {
            ...
            four_warps_per_line_SDDMM_kernel<<<160, 1024>>>(
                4,
                m,
                n,
                k,
        ...
        else if (warps_per_line > 4 && warps_per_line < 9)
        {
            ...
            eight_warps_per_line_SDDMM_kernel<<<160, 1024>>>(
                8,
                m,
                n,
                k,
        ...
        else if (warps_per_line > 8 && warps_per_line < 17)
        {
            ...
            sixteen_warps_per_line_SDDMM_kernel<<<160, 1024>>>(
                16,
                m,
                n,
                k,
