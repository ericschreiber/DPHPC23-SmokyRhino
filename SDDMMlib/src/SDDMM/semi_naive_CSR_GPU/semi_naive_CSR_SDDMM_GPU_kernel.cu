...
__device__ float warp_wise_reduction_semi_naive(float sum)
{
    ...
    float* d_result)
{
    ...
    for (int i = blockIdx.x; i < m; i += grid_size)
    {
        ...
        for (int j = d_rowPtr[i]; j < d_rowPtr[i + 1]; j++)
        {
            ...
            float warp_sum = warp_wise_reduction_semi_naive(my_sum);
            ...
            if (warp_idx == 0)
            {
                block_sum = warp_wise_reduction_semi_naive(warpSums_buffer[thread_id]);
            }
